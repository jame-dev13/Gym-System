...
public class PaymentSubscriberMetricsController {
   ...
   @GetMapping("/resume")
   public ResponseEntity<AnnualResumeResponse> getAnnualResume(final Authentication authentication) {
      return ResponseEntity.ok(paymentMetricsService.getAnnualResume(authentication));
   }

   @GetMapping("/evolution")
   public ResponseEntity<InvestmentMonthEvolutionResponse> getMonthInvestmentEvolution(final Authentication authentication) {
      return ResponseEntity.ok(paymentMetricsService.getInvestmentMonthEvolution(authentication));
   }
}
