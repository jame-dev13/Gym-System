...
import com.jame.dev.gymApp.features.customer.api.CustomerAdministrationControllerV2;
...
@WebMvcTest(
   controllers = CustomerAdministrationControllerV2.class,
   excludeFilters = {
...
class CustomerAdministrationControllerV2Test {

   @Autowired
   private MockMvc mockMvc;

   @Autowired
   private CustomerAdministrationControllerV2 controller;
   ...
   private final String URI_TEMPLATE = "/app/v2/administration/customers";
   ...
   class CustomerAdministrationControllerV2PostResourceTests {
      ...
      @Test
      @DisplayName("POST[201] Created: /app/v2/administration/customers")
      void postCustomer() throws Exception {
         given(create.create(any(CustomerCreateRequest.class))).willReturn(customerResponse);
         ...
         verify(create, times(1)).create(any(CustomerCreateRequest.class));
      }

      @Test
      @DisplayName("POST[409]: Conflict: Customer already exists")
      void customerAlreadyExists() throws Exception {
         given(create.create(any(CustomerCreateRequest.class))).willThrow(AlreadyExistsException.class);
         mockMvc.perform(post(URI_TEMPLATE)
               .contentType(MediaType.APPLICATION_JSON)
               .content(payload))
      ...
      void customerIsDeactivated() throws Exception {
         given(create.create(any(CustomerCreateRequest.class))).willThrow(NoActiveException.class);
         mockMvc.perform(post(URI_TEMPLATE)
               .contentType(MediaType.APPLICATION_JSON)
               .content(payload))
            .andExpect(status().isConflict())
      ...
      void badRequestInvalidPayloadValues() throws Exception {
         mockMvc.perform(post(URI_TEMPLATE)
               .contentType(MediaType.APPLICATION_JSON)
               .content("""
                  {
      ...
      @Test
      @DisplayName("POST[400]: Bad Request: Missing required payload field.")
      void badRequestMissingPayloadField() throws Exception {
         mockMvc.perform(post(URI_TEMPLATE)
               .contentType(MediaType.APPLICATION_JSON)
               .content("""
                  {}
   ...
   class CustomerAdministrationControllerV2PutResources {
      String payload = """
         {
      ...
      void putCustomer() throws Exception {
         given(update.update(anyLong(), any(CustomerUpdateRequest.class))).willReturn(customerResponse);
         mockMvc.perform(put(URI_TEMPLATE + '/' + 1L)
            ...
            .andExpect(jsonPath("$.*").exists())
            .andExpect(jsonPath("$.id").value(customerResponse.id()));
         verify(update, times(1)).update(anyLong(), any(CustomerUpdateRequest.class));
      }

      @Test
      @DisplayName("PUT[404] Not Found")
      void customerNotFound() throws Exception {
         given(update.update(anyLong(), any(CustomerUpdateRequest.class)))
